...
ZelzalPRO_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗧𝞝𝗣 𝗖𝗼𝗻𝗳𝗶𝗴 - أوامـر الاذا؏ـــة](t.me/tepthon) 𓆪\n\n"
    "**⎞𝟏⎝** `.للكروبات`  / `.للمجموعات`\n"
    "**بالــࢪد ؏ــلى ࢪســالة نصيــه أو وسـائــط تحتهــا نــص**\n"
    "**- لـ اذاعـة رسـالة أو ميديـا لكـل المجموعـات اللي انت موجود فيهـا . .**\n\n\n"
    "**⎞𝟐⎝** `.للخاص`\n"
    "**بالــࢪد ؏ــلى ࢪســالة نصيــه أو وسـائــط تحتهــا نــص**\n"
    "**- لـ اذاعـة رسـالة أو ميديـا لكـل الاشخـاص اللي موجـودين عنـدك خـاص . .**\n\n\n"
    "**⎞𝟑⎝** `.خاص`\n"
...
@zedub.zed_cmd(pattern="الاذاعـة")
...
async def gcast(event):
    ...
    if zedthon: #Write Code By T.me/zzzzl1l
        await edit_or_reply(event, "**⎆ بالـࢪد ؏ــلى ࢪسـالة أو وسائـط**")
        return
    elif event.is_reply:
    ...
    async for x in event.client.iter_dialogs():
        if x.is_group:
            ...
            try:
                ...
                er += 1
    await zzz.edit(
        f"**⎆ تمت الاذاعـه بنجـاح إلــى ** `{done}` **من المجموعـات** \n**⎆ خطـأ في الإرسـال إلــى ** `{er}` **من المجموعـات**"
    )
    ...
    async for x in event.client.iter_dialogs():
        if x.is_group:
            ...
            try:
                ...
                return
    await zzz.edit(
        f"**⎆ تمت الاذاعـه بنجـاح إلــى ** `{done}` **من المجموعـات ، خطـأ في الإرسـال إلــى ** `{er}` **من المجموعـات**"
    )
    ...
    async for x in event.client.iter_dialogs():
        if x.is_user and not x.entity.bot:
            ...
            try:
                ...
                return
    await zzz.edit(
        f"**⎆ تمت الاذاعـه بنجـاح إلــى ** `{done}` **من الخـاص**\n**⎆ خطـأ في الإرسـال إلــى ** `{er}` **من الخـاص**"
    )
...
async def pmto(event):
    ...
    try:
        ...
        await event.edit("**⎆ عـذرًا .. لقـد حدث خطـأ مـا .. أعـد المـحـاولـة**")
