...
class ZedVC:
    ...
    async def join_vc(self, chat, join_as=None):
        ...
        if join_as:
            try:
                ...
                return "**- قم بإضافة أيـدي المجموعـة لأمر الانضمام**"
        else:
    ...
    async def play_song(self, input, stream=Stream.audio, force=False):
        ...
        if self.PLAYING and not force:
            ...
            return f"- تم الإضافـة لـ قـائمـة التشغيـل ✓\n- المـوقـع: {len(self.PLAYLIST)+1}"
        if not self.PLAYING:
            self.PLAYLIST.append({"title": title, "path": playable, "stream": stream})
            await self.skip()
    ...
    async def pause(self):
        if not self.PLAYING:
            return "**- لايـوجـد شـي لـ الإيقاف**"
        if not self.PAUSED:
    ...
    async def resume(self):
        if not self.PLAYING:
            return "**- لايـوجـد شيء لـ الاستئنـاف **"
        if self.PAUSED:
