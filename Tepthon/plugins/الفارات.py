...
""" وصـف الملـف : أوامــر اضـافة الفـارات باللغـة العربيـة كـاملة ولا حـرف انكلـش🤘 تخمـط اذكـر المصـدر يولـد
...
ZelzalVP_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗧𝗲𝗽𝘁𝗵𝗼𝗻 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - أوامــر الفـارات](t.me/Tepthon) 𓆪\n\n"
    "**⎉╎قائمـــة اوامر تغييـر فـارات الصـور بأمـر واحـد فقـط - لـ أول مرّة على سـورس تيبثـون يوزر بـوت 🦾 :** \n\n"
    ...
    "⪼ `.اوامر الفارات` لعـرض بقيـة أوامــر الفـارات\n\n\n"
    "**⎉╎قائمـــة اوامر تغييـر كليشـة الايـدي :** \n\n"
    ...
    "**⎉╎قائمـــة اوامر تغييـر بقيـة الفـارات بأمـر واحـد فقـط :** \n\n"
    ...
    "⪼ `.جلب` + اسـم الفـار\n\n"
    "⪼ `.حذف` + اسـم الفـار\n\n"
    "⪼ `.رفع مطور` بالـرد ع الشخـص لرفعـه مطـور تحكـم كامـل بالأوامــر\n\n"
    "⪼ `.حذف فار المطورين`\n\n"
...
async def variable(event):
    ...
    # All Rights Reserved for "Tepthon" "بـاقـر"
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        if variable in heroku_var:
            await zed.edit("**⎉╎تم تغييـر {} بنجـاح ☑️**\n**⎉╎المتغيـر : ↶**\n `{}` \n**⎉╎يتم الآن إعـادة تشغيـل بـوت تيبثـــون يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str, vinfo))
        else:
            await zed.edit("**⎉╎تم إضافـة {} بنجـاح ☑️** \n**⎉╎المضاف إليـه :**\n `{}` \n**⎉╎يتم الآن إعـادة تشغيـل بـوت تيبثـــون يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str, vinfo))
...
async def variable(event):
    ...
    if input_str == "كليشة الفحص" or input_str == "الفحص":
        ...
        await zed.edit("**⎉╎تم حـذف {} بنجـاح ☑️**\n**⎉╎المتغيـر المحـذوف : ↶**\n `{}` \n**⎉╎يتم الآن إعـادة تشغيـل بـوت تيبثـــون يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str, variable))
        ...
        if gvarstatus("DIGITAL_PIC") is None:
        	return await zed.edit("**⎉╎عـذࢪاً عـزيـزي .. انت لـم تقـم باضـافـة فـار {} اصـلاً...**".format(input_str))
        await zed.edit("**⎉╎تم حـذف {} بنجـاح ☑️**\n**⎉╎المتغيـر المحـذوف : ↶**\n `{}` \n**⎉╎يتم الآن إعـادة تشغيـل بـوت ريــبــثــون يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str, variable))
        delgvar("DIGITAL_PIC")
        ...
        if variable not in heroku_var:
        	return await zed.edit("**⎉╎عـذࢪاً عـزيـزي .. انت لـم تقـم باضـافـة فـار {} اصـلاً...**".format(input_str))
        await zed.edit("**⎉╎تم حـذف {} بنجـاح ☑️**\n**⎉╎المتغيـر المحـذوف : ↶**\n `{}` \n**⎉╎يتم الآن إعـادة تشغيـل بـوت تيبثـــون يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str, heroku_var[variable]))
        ...
        if variable not in heroku_var:
        	return await zed.edit("**⎉╎عـذࢪاً عـزيـزي .. انت لـم تقـم باضـافـة فـار {} اصـلاً...**".format(input_str))
        await zed.edit("**⎉╎تم حـذف {} بنجـاح ☑️**\n**⎉╎المتغيـر المحـذوف : ↶**\n `{}` \n**⎉╎يتم الآن إعـادة تشغيـل بـوت ريــبــثــون يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str, heroku_var[variable]))
        del heroku_var[variable]
...
async def custom_zed(event):
    ...
    if (input_str == "كليشة الحماية" or input_str == "كليشة الحمايه" or input_str == "كليشه الحماية" or input_str == "كليشه الحمايه"):
        ...
        variable = "COMMAND_HAND_LER"
        if variable not in heroku_var:
            await zed.edit("**⎉╎فـار نقطـة الأوامــر غيـر موجـود ❌**\n**⎉╎لـ اضـافته بالـرد ع الرمـز فقـط استخـدم الامـر : ↶**\n `.اضف نقطة الاوامر` \n\n**⎉╎قنـاة السـورس : @Tepthon**")
        else:
            await zed.edit("**⎉╎الفـار {} موجـود ☑️**\n**⎉╎قيمـة الفـار : ↶**\n `{}` \n\n**⎉╎قنـاة السـورس : @Tepthon**".format(input_str, variable))
        ...
        if variable is None:
            await zed.edit("**⎉╎فـار صـورة الأوامــر غيـر موجـود ❌**\n**⎉╎لـ اضـافته بالـرد ع صـورة فقـط استخـدم الامـر : ↶**\n `.اضف صورة الاوامر` \n\n**⎉╎قنـاة السـورس : @Tepthon**")
        else:
            await zed.edit("**⎉╎الفـار {} موجـود ☑️**\n**⎉╎قيمـة الفـار : ↶**\n `{}` \n\n**⎉╎قنـاة السـورس : @Tepthon**".format(input_str, variable))
