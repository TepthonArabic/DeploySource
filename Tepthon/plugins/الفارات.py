...
ZelzalVP_cmd = (
    ...
    "⪼ `.اضف كليشة الايدي` بالـرد ع الكليشـه مـن القنـاة @tepthonklaesh \n\n\n"
...
async def variable(event):
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        await zed.edit("**⎉╎تم حـذف {} بنجـاح ☑️**\n**⎉╎المتغيـر المحـذوف : ↶**\n `{}` \n**⎉╎يتم الان اعـادة تشغيـل بـوت تيبثــون يستغـرق الامر 2-1 دقيقـة ▬▭ ...**".format(input_str, variable))
        ...
        if variable is None:
        	return await zed.edit("**⎉╎عـذࢪاً عـزيـزي .. انت لـم تقـم باضـافـة فـار {} اصـلاً...**".format(input_str))
        await zed.edit("**⎉╎تم حـذف {} بنجـاح ☑️**\n**⎉╎المتغيـر المحـذوف : ↶**\n `{}` \n**⎉╎يتم الان اعـادة تشغيـل بـوت تيبثــون يستغـرق الامر 2-1 دقيقـة ▬▭ ...**".format(input_str, heroku_var[variable]))
        delgvar(variable)
    elif input_str == "رسائل الحماية" or input_str == "رسائل الحمايه" or input_str == "رسائل الخاص" or input_str == "رسائل حماية الخاص" or input_str == "عدد التحذيرات":
        variable = "MAX_FLOOD_IN_PMS"
        await asyncio.sleep(1.5)
...
async def custom_zed(event):
    ...
    if (input_str == "كليشة الحماية" or input_str == "كليشة الحمايه" or input_str == "كليشه الحماية" or input_str == "كليشه الحمايه"):
        ...
        if variable is None:
            await zed.edit("**⎉╎فـار ايموجي/رمز الايدي غيـر موجـود ❌**\n**⎉╎لـ اضـافته بالـرد ع الكليشـة من هنا ( @tepthonklaesh ) استخـدم الامـر : ↶**\n `.اضف كليشة الايدي` \n\n**⎉╎قنـاة السـورس : @Tepthon**")
        else:
            await zed.edit("**⎉╎الفـار {} موجـود ☑️**\n**⎉╎قيمـة الفـار : ↶**\n `{}` \n\n**⎉╎قنـاة السـورس : @Tepthon**".format(input_str, variable))
...
if Config.HEROKU_API_KEY is None:
    ...
    zed = await edit_or_reply(event, "**⎉╎جـاري إعداد المنطقـة الزمنيـة لـ تيبثــون 🌐...**")
    # All Rights Reserved for "Zed-Thon" "زلـزال الهيبـه"
...
@zedub.zed_cmd(pattern="خلي الاسم(?:\\s|$)([\\s\\S]*)")
