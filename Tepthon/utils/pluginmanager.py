...
def remove_plugin(shortname):
        ...
        for cmdname in cmd:
            if cmdname in LOADED_CMDS:
                for i in LOADED_CMDS[cmdname]:
                    tepthon.remove_event_handler(i)
    ...
    try:
        name = f"tepthon.plugins.{shortname}"
        for i in reversed(range(len(tepthon._event_builders))):
            ev, cb = tepthon._event_builders[i]
            if cb.__module__ == name:
                del tepthon._event_builders[i]
