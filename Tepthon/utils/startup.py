...
async def setup_bot():
    tepthon.me = await tepthon.get_me()
    tepthon.uid = tepthon.me.id
    if Config.OWNER_ID == 0:
        Config.OWNER_ID = get_peer_id(tepthon.me)
    await tepthon.tgbot.start(bot_token=Config.TG_BOT_USERNAME)
    tepthon.tgbot.me = await tepthon.tgbot.get_me()
    bot_details = tepthon.tgbot.me
...
async def saves():
    ...
    try:
        ...
        await tepthon(JoinChannelRequest("@P17_12"))
...
async def mybot():
    tepthon_USER = tepthon.me.first_name
    The_razan = tepthon.uid
    rz_ment = f"[{tepthon_USER}](tg://user?id={The_razan})"
    f"ـ {rz_ment}"
    f"⪼ هذا هو بوت خاص بـ {rz_ment} يمكنك التواصل معه هنا"
    starkbot = await tepthon.tgbot.get_me()
    perf = "[ تيبثون ]"
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            await tepthon.send_message("@BotFather", "/setinline")
            await asyncio.sleep(1)
            await tepthon.send_message("@BotFather", botname)
            await asyncio.sleep(1)
            await tepthon.send_message("@BotFather", perf)
...
async def startupmessage():
    ...
    try:
        if BOTLOG:
            Config.JMTHONLOGO = await tepthon.tgbot.send_file(
                BOTLOG_CHATID,
    ...
    try:
        if msg_details:
            await tepthon.check_testcases()
            message = await tepthon.get_messages(msg_details[0], ids=msg_details[1])
            text = message.text + "\n\n**الان السورس شغال طبيعي.**"
            await tepthon.edit_message(msg_details[0], msg_details[1], text)
...
async def add_bot_to_logger_group(chat_id):
    ...
    """
    bot_details = await tepthon.tgbot.get_me()
    try:
        await tepthon(
...
async def load_plugins(folder, extfolder=None):
    ...
    if extfolder:
        if not failure:
            failure.append("None")
        await tepthon.tgbot.send_message(
            BOTLOG_CHATID,
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        try:
            entity = await tepthon.get_entity(BOTLOG_CHATID)
        ...
        photobt = await tepthon.upload_file(file="razan/pic/tepthon.jpeg")
        _, groupid = await create_supergroup(
            "كروب بوت تيبثون", tepthon, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        try:
            entity = await tepthon.get_entity(PM_LOGGER_GROUP_ID)
            if not isinstance(entity, types.User) and not entity.creator:
                if entity.default_banned_rights.send_messages:
        ...
        photobt = await tepthon.upload_file(file="razan/pic/tepthon.jpeg")
        _, groupid = await create_supergroup(
            "مجموعة التخزين", tepthon, Config.TG_BOT_USERNAME, descript, photobt
        )
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    if response.code != 200:
        LOGS.error(errtext)
        return await tepthon.tgbot.send_message(BOTLOG_CHATID, errtext)
    await runcmd(gcmd)
    if not os.path.exists(cfolder):
        LOGS.error(
            "هنالك خطأ اثناء استدعاء رابط الملفات الاضافية يجب التأكد من الرابط اولا "
        )
        return await tepthon.tgbot.send_message(
            BOTLOG_CHATID,
            "هنالك خطأ اثناء استدعاء رابط الملفات الاضافية يجب التأكد من الرابط اولا ",
        )
